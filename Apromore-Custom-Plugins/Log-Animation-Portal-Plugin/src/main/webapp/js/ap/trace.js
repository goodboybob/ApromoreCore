...
// tokenAnimation is expected to one of the elements of this.jsonServer.logs[].tokenAnimations
...
let LogCase = {
    construct: function(controller, tokenAnimation, color, label, offset) {
        ...
        this.svgDocument = controller.svgDocument;
    ...
    getPathElement: function(path) {
        ...
        if (!pathElement) {
            ...
            pathElement = this.pathElementCache[path.id] = $j(
    ...
    createNodeMarker: function(t, dt, node, begin, dur, offset) {
        ...
        let incomingEndPoint = $j(
            "[data-element-id=" +
            this.controller.canvas.getIncomingFlowId(node.id) +
            "]"
        );
        //console.log(incomingEndPoint);
        let incomingPathE = incomingEndPoint
            .find("g")
            .find("path")
            .get(0);
        ...
        let outgoingStartPoint = $j(
            "[data-element-id=" +
            this.controller.canvas.getOutgoingFlowId(node.id) +
            "]"
        );
        let outgoingPathE = outgoingStartPoint
            .find("g")
            .find("path")
            .get(0);
        ...
        if (node.isVirtual == "false") {
            ...
            let path =
                ...
                "," +
                endPoint.y;
            //let pathId = node.id +"_path";
        } else {
            //let pathId = node.id +"_virtualpath";
            ...
            if (
                ...
                let arrayAbove = new Array();
                let arrayBelow = new Array();
